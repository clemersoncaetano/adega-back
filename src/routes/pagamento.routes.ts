...
import { criarPedido, PedidoItemInput } from "../models/pedido.model";
...
function normalizarItensPedido(itens: unknown): CheckoutItemInput[] {
  ...
  return itens.map((item) => {
    ...
    const checkoutItem = item as CheckoutItemInput;
    ...
    if (
      !Number.isInteger(checkoutItem.quantidade) ||
...
router.post("/pagamentos/preferencia", async (req, res) => {
  try {
    ...
    const itensPedido: PedidoItemInput[] = [];
    ...
    const { pedidoId, total } = await criarPedido({
      nome,
      email,
      itens: itensPedido,
    });
    ...
    const statusCode =
      message === "MP_ACCESS_TOKEN nao configurado"
        ? 503
